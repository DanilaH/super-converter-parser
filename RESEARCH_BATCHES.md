...
8. On supported versioned global-admission forks (`v1` or `v1.1`), raw Related evidence is copied but prior-generation `selectedForExpansion` flags are reset. The new generation writes its own derived selection only when its current global frontier is materialized, using the persisted admission version for that generation. Legacy no-marker immediate-expansion runs preserve their historical behavior.
