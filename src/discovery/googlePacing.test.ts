...
test('conservativeLegacyHoldMs preserves removed serial Surfer time without inventing a fixed cadence', () => {
  assert.equal(conservativeLegacyHoldMs(sample()), 4800);
...
test('GoogleLegacyCadencePacer lets downstream work consume the hold before the next navigation', async () => {
  ...
  assert.equal(waited, 2800);
  assert.deepEqual(sleeps, [2800]);
