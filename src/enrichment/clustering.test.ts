...
test('exact match: identical domain and URL sets merge into one cluster', () => {
  ...
  assert.equal(result.pairs[0]?.classification, 'strong');
});

test('partial overlap: domain and URL thresholds gate edge creation', () => {
  const inputs = withIds([
...
test('below domain Jaccard threshold: no edge, two singletons', () => {
  const inputs = withIds([
    { keyword: 'json diff', normalizedKeyword: 'json diff', volume: 800, domains: ['a.com', 'b.com', 'c.com', 'd.com', 'e.com'] },
...
test('complete-link merges a three-keyword clique', () => {
  const inputs = withIds([
  ...
  assert.equal(result.clusters[0]!.cohesion?.pairCount, 3);
});
...
test('singleton: keyword with no edges forms its own cluster', () => {
  ...
  for (const cluster of result.clusters) {
    ...
    assert.deepEqual(cluster.cohesion, { pairCount: 0, urlJaccard: null, domainJaccard: null });
  }
});
...
test('missing SERP: keywords without domains are excluded with reason', () => {
  const inputs = withIds([
    ...
    { keyword: 'no serp', normalizedKeyword: 'no serp', volume: 200, domains: [], urls: [] },
  ]);
  const result = clusterKeywords(inputs, DEFAULT_CONFIG);
...
test('configurable domain thresholds change results', () => {
  const inputs = withIds([
  ...
  const strict = clusterKeywords(inputs, { topN: 10, edgeRule: { minSharedDomains: 4, minJaccard: 0.5, minSharedUrls: 2, minUrlJaccard: 0.1 }, algorithmVersion: CLUSTERING_ALGORITHM_VERSION });
  const lenient = clusterKeywords(inputs, { topN: 10, edgeRule: { minSharedDomains: 2, minJaccard: 0.2, minSharedUrls: 2, minUrlJaccard: 0.1 }, algorithmVersion: CLUSTERING_ALGORITHM_VERSION });
  assert.equal(strict.clusters.length, 2);
  assert.equal(lenient.clusters.length, 1);
});

test('topN truncates domain and URL comparison windows', () => {
  const inputs = withIds([
  ...
  const top3 = clusterKeywords(inputs, { topN: 3, edgeRule: { minSharedDomains: 3, minJaccard: 0.5, minSharedUrls: 2, minUrlJaccard: 0.1 }, algorithmVersion: CLUSTERING_ALGORITHM_VERSION });
  const top5 = clusterKeywords(inputs, { topN: 5, edgeRule: { minSharedDomains: 3, minJaccard: 0.5, minSharedUrls: 2, minUrlJaccard: 0.1 }, algorithmVersion: CLUSTERING_ALGORITHM_VERSION });
...
test('source keyword index owns relations even when normalized text collides', () => {
  const inputs: ClusteringInput[] = [
    withUrls({ keywordIdx: 10, keyword: 'JSON Diff', normalizedKeyword: 'json diff', volume: 100, domains: ['a.com', 'b.com', 'c.com'] }),
    withUrls({ keywordIdx: 20, keyword: 'json   diff', normalizedKeyword: 'json diff', volume: 200, domains: ['a.com', 'b.com', 'c.com'] }),
  ];
  const result = clusterKeywords(inputs, DEFAULT_CONFIG);
...
test('duplicate source keyword index is rejected instead of silently merging relations', () => {
  const inputs: ClusteringInput[] = [
    withUrls({ keywordIdx: 7, keyword: 'one', normalizedKeyword: 'one', volume: 100, domains: ['a.com'] }),
    withUrls({ keywordIdx: 7, keyword: 'two', normalizedKeyword: 'two', volume: 200, domains: ['b.com'] }),
  ];
...
test('large complete-link clique uses bounded pair lookup', () => {
  const inputs: ClusteringInput[] = Array.from({ length: 200 }, (_, index) => withUrls({
    keywordIdx: index,
