...
async function seed(directory: string): Promise<void> {
  ...
  for (const artifact of [...ENTRANT_COHORT_ARTIFACTS, ...COHORT_HISTORY_ARTIFACTS]) {
    await writeFile(join(directory, artifact), 'stale');
  }
}

test('representative revision change removes entrant and cohort-history publication transitively', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'representative-history-cascade-'));
  try {
