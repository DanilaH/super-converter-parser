...
export async function writeCohortHistoricalPresenceCsv(
  ...
  const rows = snapshot.collection.domains.map((domain) => {
    ...
    return [
      ...
      domain.omitReason ?? '',
      domain.priority.isWeak === null || domain.priority.isWeak === undefined ? '' : String(domain.priority.isWeak),
      domain.priority.drStatus ?? '',
      domain.priority.dr === null || domain.priority.dr === undefined ? '' : String(domain.priority.dr),
      String(domain.priority.bestRank),
      String(domain.priority.occurrenceCount),
      String(domain.priority.clusterCount),
      domain.priority.queryCount === undefined ? '' : String(domain.priority.queryCount),
      domain.priority.distinctPageCount === undefined ? '' : String(domain.priority.distinctPageCount),
      domain.cacheStatus,
      result?.status ?? '',
