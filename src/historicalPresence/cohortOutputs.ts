...
export async function writeCohortHistoricalPresenceCsv(
  ...
  const rows = snapshot.collection.domains.map((domain) => {
    ...
    return [
      ...
      domain.omitReason ?? '',
      domain.priority.isWeak === null ? '' : String(domain.priority.isWeak),
      domain.priority.drStatus,
      domain.priority.dr === null ? '' : String(domain.priority.dr),
      String(domain.priority.bestRank),
      String(domain.priority.occurrenceCount),
      String(domain.priority.clusterCount),
      String(domain.priority.queryCount),
      String(domain.priority.distinctPageCount),
      domain.cacheStatus,
      result?.status ?? '',
