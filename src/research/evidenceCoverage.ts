...
function projectHistoryCoverage(
  ...
  if (firstSeenKnownDomainCount < cohortDomainCount) {
    warnings.push({
      ...
      denominator: cohortDomainCount,
      message: `${firstSeenKnownDomainCount}/${cohortDomainCount} finalist entrant-domain row(s) have observed web first-seen evidence. Rows without a known first-seen date must not be interpreted as established/old.`,
    });
  }
